...
**Decisão atual:** PRONTO PARA AUTORIZAÇÃO LITERAL<br>
...
**Produção:** não alterada; workflow bloqueado até a frase literal do SHA completo
...
| Backend produtivo | Supabase exclusivo, backups, RLS, migrations e funções aprovadas                    | projeto Free isolado; credenciais protegidas; schema pendente ao workflow autorizado |
...
| Autorização       | registro `G12_<sha>.json` v2 e `AUTORIZO-G12-PRODUCAO:<sha>`                        | falta somente a frase literal do SHA `e52b25d…`                                      |
